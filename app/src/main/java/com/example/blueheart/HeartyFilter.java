...
public class HeartyFilter {
     ...
     *            thrown.
     */
    public HeartyFilter(float[] b_taps, float[] a_taps) {
    ...
    public HeartyFilter(float b0, float b1, float b2, float b3, float b4,
    ...
    public static class MeanFilter extends HeartyFilter {
         ...
         * (non-Javadoc)
         *
         * @see de.lme.plotview.HeartyFilter#next(float)
         */
        @Override
        public float next(float xnow) {
            y[1] = y[0];
    ...
    public static class AccuFilter extends WndIntFilter {
        ...
        private float sumVal(List<Float> m_int) {
            float sum = 0 ;
            for (int i = 0; i < m_int.size(); i++) {
                sum += m_int.get(i) ;
            }
